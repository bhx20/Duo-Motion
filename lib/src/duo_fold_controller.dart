...
class DuoFoldController extends ChangeNotifier {
  ...
  double? _gestureLiftDirX;
  double? _gestureLiftDirY;

  /// Sets manual tilt and optionally an explicit lift direction (used by gesture interactions).
  void setManualTilt(double degrees, {ui.Offset? liftDirection}) {
    final clamped = degrees
        .clamp(-FoldMotionModel.maxTiltDegrees, FoldMotionModel.maxTiltDegrees)
        .toDouble();
    final newDirX = liftDirection?.dx;
    final newDirY = liftDirection?.dy;

    if (clamped == _manualTiltDegrees &&
        newDirX == _gestureLiftDirX &&
        newDirY == _gestureLiftDirY) {
      return;
    }

    _manualTiltDegrees = clamped;
    _gestureLiftDirX = newDirX;
    _gestureLiftDirY = newDirY;
    if (!useSensor) {
      notifyListeners();
    }
  }
  ...
  double get _rawLiftDirX {
    if (useSensor) return _sensorState.liftDirX;
    if (_gestureLiftDirX != null) return _gestureLiftDirX!;
    return _manualTiltDegrees >= 0 ? -1.0 : 1.0;
  }

  double get _rawLiftDirY {
    if (useSensor) return _sensorState.liftDirY;
    if (_gestureLiftDirY != null) return _gestureLiftDirY!;
    return 0.0;
  }
  ...
  void recalibrate() {
    ...
    _gestureLiftDirX = null;
    _gestureLiftDirY = null;
    notifyListeners();
  }
