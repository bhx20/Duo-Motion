...
/// no-op that reads as "the package is broken", duo_motion fails loudly.
class DuoFoldUnsupportedError implements Exception {
  ...
  String get message =>
      'duo_motion requires the Impeller renderer on Android or iOS: $reason. '
...
/// Thrown when an invalid or unsupported fold mode configuration is supplied.
class DuoFoldModeError implements Exception {
  /// Creates a fold mode error.
  const DuoFoldModeError(this.message);

  /// Details of the mode error.
  final String message;

  @override
  String toString() => 'DuoFoldModeError: $message';
}
